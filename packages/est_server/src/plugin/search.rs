...
use crate::{engine::{self, ArenaEngine, Engine, HandleEngine, SearchAction, SearchResult}, query::Query, router::{self, Router, RouterFunc, RouterMapLeaf}};
...
impl PluginSearch {
    ...
    pub fn router(
        ...
        let engine_ddg = arena.insert(EngineDuckDuckGo { is_bang: false, is_direct: false });
        let engine_ddg_bang = arena.insert(EngineDuckDuckGo { is_bang: true, is_direct: false });
        let engine_ddg_direct = arena.insert(EngineDuckDuckGo { is_bang: false, is_direct: true });
        ...
        let map = RouterMapLeaf::new(HashMap::from([
        ...
        let fallback = RouterFunc(move |query| Ok(Some(fallback(query))));
...
impl Engine for EngineBing {
    fn search(&self, query: &Query) -> SearchResult
    {
        if self.is_china || query.mention().get(1) == Some(&"cn") {
...
impl Engine for EngineChina {
    ...
    {
        match self {
            EngineChina::Baidu => Ok(SearchAction::redirect_to_query(
                "https://www.baidu.com/s",
                &[("wd", query.content())],
            )),
            EngineChina::Sogou => Ok(SearchAction::redirect_to_query(
...
impl Engine for EngineWikipedia {
    fn search(&self, query: &Query) -> SearchResult
    {
        let lang = match query.mention().residue().first() {
...
impl Engine for EngineYixuanFavorite {
    ...
    {
        match self {
            EngineYixuanFavorite::GoogleScholar => Ok(SearchAction::redirect_to_query(
                "https://scholar.google.com/scholar",
                &[("q", query.content())],
            )),
            EngineYixuanFavorite::Arxiv => Ok(SearchAction::redirect_to_query(
                "https://arxiv.org/search",
                &[("query", query.content())],
            )),
            EngineYixuanFavorite::OpenReview => Ok(SearchAction::redirect_to_query(
                "https://openreview.net/search",
                &[("query", query.content())],
            )),
            EngineYixuanFavorite::Github => Ok(SearchAction::redirect_to_query(
                "https://github.com/search",
                &[("q", query.content())],
            )),
            EngineYixuanFavorite::Wiktionary => Ok(SearchAction::redirect_to_query(
                "https://en.wiktionary.org/w/index.php",
                &[("search", query.content())],
            )),
            EngineYixuanFavorite::MerriamWebster => Ok(SearchAction::redirect_to(
                &url::Url::parse(&format!(
                    "https://www.merriam-webster.com/dictionary/{}",
                    query.content()
                )).map(|url| url.to_string()).unwrap_or_else(|_| "https://www.merriam-webster.com".to_string()),
            )),
            EngineYixuanFavorite::OED => Ok(SearchAction::redirect_to_query(
                "https://www.oed.com/search/dictionary",
                &[("scope", "Entries"), ("q", query.content())],
            )),
            EngineYixuanFavorite::Perplexity => Ok(SearchAction::redirect_to_query(
                "https://www.perplexity.ai/search/new",
                &[("q", query.content())],
            )),
            EngineYixuanFavorite::ZiTools => Ok(SearchAction::redirect_to(
                &url::Url::parse(&format!(
                    "https://zi.tools/zi/{}",
                    query.content()
                )).map(|url| url.to_string()).unwrap_or_else(|_| "https://zi.tools".to_string()),
            )),
            EngineYixuanFavorite::Jisho => {
                let url = url::Url::parse(&format!(
                    "https://jisho.org/search/{}",
                    query.content()
                )).map(|url| url.to_string()).unwrap_or_else(|_| "https://jisho.org".to_string());
                Ok(SearchAction::redirect_to(&url))
            },
            EngineYixuanFavorite::Zhihu => Ok(SearchAction::redirect_to_query(
                "https://www.zhihu.com/search",
                &[("q", query.content())],
            )),
            EngineYixuanFavorite::Douban => Ok(SearchAction::redirect_to_query(
                "https://www.douban.com/search",
                &[("q", query.content())],
            )),
            EngineYixuanFavorite::Bilibili => Ok(SearchAction::redirect_to_query(
                "https://search.bilibili.com/all",
                &[("keyword", query.content())],
            )),
            EngineYixuanFavorite::Weibo => Ok(SearchAction::redirect_to_query(
                "https://s.weibo.com/weibo",
                &[("q", query.content())],
            )),
            EngineYixuanFavorite::Xiaohongshu => Ok(SearchAction::redirect_to_query(
                "https://www.xiaohongshu.com/search_result",
                &[("keyword", query.content())],
            )),
            EngineYixuanFavorite::YouTube => Ok(SearchAction::redirect_to_query(
